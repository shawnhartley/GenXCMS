...
<cfset getAllRecords = event.getlinksCategories()>
...
<div><form action="#BuildURL(event=url.event,action=url.action, args='function=add')#" method="post"><input type="text" name="title" /><button type="submit">Add Link Category</button></form></div><br /><br />
...
<table id="users" border="0" cellspacing="0">
	      ...
	      <cfloop query="getAllRecords">
	                ...
	                <td><cfif showOnsite><a href="#BuildURL(event=url.event,action=url.action, id=id, args='function=deactivate')#"><img src="#application.slashroot#managementcenter/images/main/active.gif"></a><cfelse>
