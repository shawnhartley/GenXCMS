...
var getFileInfo = function(file) {
	...
	template += '<button id="edit">Edit Image</button>';
	template += '</form>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			$('#edit').click(function() {
				document.write('<input id="el" type="button" onclick="window.location.reload();" value="Return the the Main File Manager Page"/>');
				document.write('<iframe id="closer" src="scripts/index.cfm?filename=' + data['Path'] + '" height="100%" width="100%"></iframe>');
			});
