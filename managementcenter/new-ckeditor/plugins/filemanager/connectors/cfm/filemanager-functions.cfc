...
---><cfcomponent output="no" hint="functions for the cfml filemanager connector">
	...
	<cffunction name="addFile" returntype="void" access="public">
		...
		<!--- upload the file --->
		<cftry>
        <cfif NOT DirectoryExists( ExpandPath( '/scratch/' ) )>
        <cfdirectory action="create" directory="#expandPath('/scratch/')#">
        </cfif>

			<cffile action="upload" destination="#expandPath('/scratch/')#" filefield="#formfieldname#" nameconflict="makeunique" result="file_struct" />        
