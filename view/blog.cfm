...
<cfset pagination.setQueryToPaginate(event.blog.listing) />
...
<cfoutput query="event.blog.listing" startrow="#pagination.getStartRow()#" maxrows="#pagination.getMaxRows()#">
	<cfif isNumeric(event.slug)>
		<cfif event.slug eq id>
		    <a href="/blog/" class="button small">Back to blog</a>
		    <h1>#headline#</h1>
		    #content#
		</cfif>
	<cfelse>
		<h2 class="blogHeadline"><a href="#BuildURL(event=settings.var('blog.blogurl') & '/#id#')#">#headline#</a></h2>
		#summary#
		<hr />
	</cfif>
</cfoutput>

<cfif settings.var('modules') CONTAINS 'Comments' AND isNumeric(event.slug) AND event.blog.listing.recordcount EQ 1>
	<cfset comments = event.comments.getComments(commentOn = 'blog', refId = event.slug)>
	...
	<cfelse>
		...
		<b class="clear"><input type="hidden" name="commentOn" value="blog" />
			<button class="comment" name="refId" value="#event.blog.listing.id#" type="submit">Comment</button></b>
		</cfoutput>
		</form><br /><br />
	</cfif>

<cfelse>
	<cfif event.blog.listing.recordcount>
		<cfoutput>#pagination.getRenderedHTML()#</cfoutput>
	</cfif>
	<cfif NOT event.blog.listing.recordcount>
...
<div id="blogList">
<ul><cfset allblog = event.blog.getAll()>
<cfoutput query="allblog"><cfset liclass = ''><cfif event.slug EQ id><cfset liclass = 'class="active"'></cfif>
<li #liclass#><a href="#BuildURL(event=settings.var('blog.blogurl') & '/#id#')#">#Left(headline, 30)#<cfif len(headline) GT 30>&hellip;</cfif></a></li>
</cfoutput>
<li><cfoutput><a href="#BuildURL(event=settings.var('blog.blogurl'))#">View All</a></cfoutput></li>
</ul>
</div>
