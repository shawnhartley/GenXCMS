...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->
	...
	<cffunction name="init" access="public" output="false" returntype="MailService" hint="Constructor">
		<cfargument name="mailSettings" type="logbox.system.core.mail.MailSettingsBean" required="false" hint="A configured mail settings bean with default mail configurations, else ignored and uses payload"/>
		...
		<cfscript>
			...
			if( structKeyExists(arguments,"mailSettings") ){
				...
				variables.mailSettings = createObject("component","logbox.system.core.mail.MailSettingsBean").init();
			}
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!--- Mail Settings --->
	<cffunction name="getMailSettingsBean" output="false" access="public" returntype="logbox.system.core.mail.MailSettingsBean" hint="Get the mail settings configuration object">
		<cfreturn variables.mailSettings>    	
    </cffunction>

	<!--- newMail --->
	<cffunction name="newMail" access="public" returntype="logbox.system.core.mail.Mail" output="false" hint="Get a new Mail payload object, just use config() on it to prepare it or pass in all the arguments via this method">
		<cfscript>
			return createObject("component","logbox.system.core.mail.Mail").init(argumentCollection=arguments);
		</cfscript>
	</cffunction>
	...
	<cffunction name="send" access="public" returntype="struct" output="false" hint="Send an email payload. Returns a struct: [error:boolean,errorArray:array]">
		<cfargument name="mail" required="true" type="logbox.system.core.mail.Mail" hint="The mail payload to send." />
		<cfscript>
...
<!------------------------------------------- PRIVATE ------------------------------------------->
	...
	<cffunction name="mailIt" output="false" access="private" returntype="void" hint="Mail a payload">
		<cfargument name="mail" required="true" type="logbox.system.core.mail.Mail" hint="The mail payload" />
		<cfscript>
			// Determine Mail Type?
